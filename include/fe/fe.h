...
/*template <>
Real FE<2,SUBDIV>::shape_third_deriv(const Elem* elem,
              const Order order,
              const unsigned int i,
              const unsigned int j,
              const Point& p);
...
public:
  ...
  static Real regular_shape_second_deriv(const unsigned int i,
    ...
    /**
     * @returns the third \f$ j^{th} \f$ derivative of the
     * \f$ i^{th} \f$ of the 12 quartic box splines interpolating
     * a regular Loop subdivision element, evaluated at the
     * barycentric coordinates \p v, \p w. Convention: j=0: d3phidv^3,j=1: d3phidv^2w,
     * j=2: d3phidw^2v, j=3: d3phidw^3
     */
    static Real regular_shape_third_deriv(const unsigned int i,
                                           const unsigned int j,
                                           const Real v,
                                           const Real w);

    static Real shape_third_deriv(const ElemType type,
                                            const Order order,
                                            const unsigned int i,
                                            const unsigned int j,
                                  const Point& p);
    static Real shape_third_deriv(const Elem* elem,
                                     const Order order,
                                     const unsigned int i,
                                     const unsigned int j,
                           const Point& p);
   ...
   * of the Loop limit mask for evaluating the limit surface at
   * a node explicitly. The size of \p weights will be
   ...
   */
  static void get_limit_mask(std::vector<Real> & weights,
  ...
  static void init_subdiv_matrix(DenseMatrix<Real> &A,
    ...
    /**
     * @returns the shape function third derivatives at the quadrature
     * points
     */
    const std::vector<std::vector<OutputShape> >& get_d3phidxi3() const
    { libmesh_assert(!calculations_started || calculate_d2phi);
        calculate_d2phi = calculate_dphiref = true; return d3phidxi3; }
    
    /**
     * @returns the shape function third derivatives at the quadrature
     * points
     */
    const std::vector<std::vector<OutputShape> >& get_d3phideta3() const
    { libmesh_assert(!calculations_started || calculate_d2phi);
        calculate_d2phi = calculate_dphiref = true; return d3phideta3; }
    /**
     * @returns the shape function third derivatives at the quadrature
     * points
     */
    const std::vector<std::vector<OutputShape> >& get_d3phidxi2eta() const
    { libmesh_assert(!calculations_started || calculate_d2phi);
        calculate_d2phi = calculate_dphiref = true; return d3phidxi2eta; }
    
    /**
     * @returns the shape function third derivatives at the quadrature
     * points
     */
    const std::vector<std::vector<OutputShape> >& get_d3phideta2xi() const
    { libmesh_assert(!calculations_started || calculate_d2phi);
        calculate_d2phi = calculate_dphiref = true; return d3phideta2xi; }
    
    /**
     * @returns the third partial derivatives.
     */
    const std::vector<RealGradient>& get_d3xyzdxi3() const
    { return (dynamic_cast<FESubdivMap*>(this->_fe_map.get()))->get_d3xyzdxi3() ; }
    const std::vector<RealGradient>& get_d3xyzdeta3() const
    { return (dynamic_cast<FESubdivMap*>(this->_fe_map.get()))->get_d3xyzdeta3() ; }
    const std::vector<RealGradient>& get_d3xyzdxi2eta() const
    { return (dynamic_cast<FESubdivMap*>(this->_fe_map.get()))->get_d3xyzdxi2eta() ; }
    const std::vector<RealGradient>& get_d3xyzdeta2xi() const
    { return (dynamic_cast<FESubdivMap*>(this->_fe_map.get()))->get_d3xyzdeta2xi() ; }
    
protected:
    
    /**
     * Shape function third derivatives in the xi (=v) direction.
     */
    std::vector<std::vector<OutputShape> >   d3phidxi3;
    
    /**
     * Shape function third derivatives in the eta (=w) direction.
     */
    std::vector<std::vector<OutputShape> >   d3phideta3;
    
    /**
     * Shape function third derivatives in the xi^2,eta (=v^2w) direction.
     */
    std::vector<std::vector<OutputShape> >   d3phidxi2eta;
    
    /**
     * Shape function third derivatives in the eta^2xi (=w^2v) direction.
     */
    std::vector<std::vector<OutputShape> >   d3phideta2xi;
