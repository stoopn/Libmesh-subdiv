...
void MeshTools::Subdiv::find_one_ring(const Tri3SD* elem, std::vector<Node*>& nodes)
{
	...
	libmesh_assert(elem->get_ordered_node(0) != NULL);

	unsigned int valence = elem->get_ordered_node(0)->valence();
...
void MeshTools::Subdiv::all_subdiv(MeshBase& mesh)
{
	...
	if (mesh_has_boundary_data)
	{
		...
		libmesh_assert(new_boundary_elements.size() > 0);

		// We should also be sure that the lengths of the new boundary data vectors
		// are all the same.
		libmesh_assert(new_boundary_sides.size() == new_boundary_elements.size());
		libmesh_assert(new_boundary_sides.size() == new_boundary_ids.size());
	...
	{ 
		...
		for (unsigned int i = 0; i < elem->n_sides(); ++i)
		{
			if (elem->neighbor(i) == NULL) 
			{
				...
				if (elem->neighbor(next[i]) != NULL)
				{
					Tri3SD* nb = static_cast<Tri3SD*>(elem->neighbor(next[i]));
					nb->set_ghost(true);
				}
				if (elem->neighbor(prev[i]) != NULL)
				{
...
void MeshTools::Subdiv::add_boundary_ghosts(MeshBase& mesh)
{
	...
	{   
		...
		for (unsigned int i = 0; i < elem->n_sides(); ++i)
		{
			libmesh_assert(elem->neighbor(i) != elem);
			if (elem->neighbor(i) == NULL)
			{
